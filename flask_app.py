...
app = Flask(__name__)
app.secret_key = 'ineedana'
app.config['SQLALCHEMY_DATABASE_URI']='sqlite:////home/nosagielang/mysite/tmp/database.db'
db=SQLAlchemy(app)
...
def listing_edit(listing_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            abort(404)
        return render_template('newlisting.html', form=lstForm)
...
def listing_delete(listing_id):
    ...
    db.session.commit()
    return render_template('listing_delete_confirm.html', listingid=listing_id)
